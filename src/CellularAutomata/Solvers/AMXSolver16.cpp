...
AMXSolver16::AMXSolver16(CADataDomain<uint8_t> *domain, CADataDomain<uint8_t> *domainBuffer, int nThreads)
{
    ...
    int nTilesPerRow = n / 16;
    int tilesRowPerThread = (nTilesPerRow + nThreads - 1) / nThreads;
    dataI = new uint8_t *[nThreads];
    for (int i = 0; i < nThreads; i++)
    {
        dataI[i] = new uint8_t[(nTilesPerRow * 16 + 2 * 16) * (tilesRowPerThread * 16 + 16 * 2)];
    }
    lDebug(1, "Allocating %llu bytes for %i threads", (nTilesPerRow * 16 + 2 * 16) * (tilesRowPerThread * 16 + 16 * 2), nThreads);
...
#pragma omp barrier
    ...
    {
        size_t i = iter * 16;
        if ((thread_id * tilesRowPerThread * 16 + i) >= nWithHalo)
            continue;
        ...
        for (size_t j = 0; j < nWithHalo - 16 * 2; j += 16)
        {
            ...
            _tile_loadd(7, &data[(thread_id * tilesRowPerThread * 16 + i) * nWithHalo + j], nWithHalo);
            _tile_dpbssd(0, 7, 3);

            _tile_loadd(7, &data[(thread_id * tilesRowPerThread * 16 + i) * nWithHalo + j + 16], nWithHalo);
            _tile_dpbuud(0, 7, 2);

            _tile_loadd(7, &data[(thread_id * tilesRowPerThread * 16 + i) * nWithHalo + j + 32], nWithHalo);
            _tile_dpbssd(0, 7, 1);
            ...
            for (int ii = 0; ii < 16; ii++)
            {
                for (int jj = 0; jj < 16; jj++)
                {
                    dataI[thread_id][(i + jj) * nWithHalo + j + ii + 16] = buffer[thread_id * 16 * 16 + ii * 16 + jj];
                }
            }
    ...
    {
        size_t i = iter * 16;
        if ((thread_id * tilesRowPerThread * 16 + i + 32) >= nWithHalo)
            continue;
        ...
        for (int j = 0; j < nWithHalo - 16 * 2; j += 16)
        {
            ...
            _tile_loadd(7, &dataI[thread_id][(i)*nWithHalo + j + 16], nWithHalo);
            _tile_dpbuud(0, 7, 3);

            _tile_loadd(7, &dataI[thread_id][(i + 16) * nWithHalo + j + 16], nWithHalo);
            _tile_dpbuud(0, 7, 2);

            _tile_loadd(7, &dataI[thread_id][(i + 32) * nWithHalo + j + 16], nWithHalo);
            _tile_dpbssd(0, 7, 1);
            ...
            for (int ii = 0; ii < 16; ii++)
            {
                ...
                {
                    uint8_t cellValue = data[(thread_id * tilesRowPerThread * 16 + i + ii + 16) * nWithHalo + j + 16 + jj];
                    ...
                    dataBuffer[(thread_id * tilesRowPerThread * 16 + i + ii + 16) * nWithHalo + j + 16 + jj] = result;
