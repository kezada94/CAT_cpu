...
AMXSolver::AMXSolver(CADataDomain<uint8_t> *domain, CADataDomain<uint8_t> *domainBuffer, int nThreads)
{
	...
	size_t n = dataDomain->getInnerHorizontalSize();
	int nTilesPerRow = n / 64;
	int tilesRowPerThread = (nTilesPerRow + nThreads - 1) / nThreads;
	dataI = new uint8_t *[nThreads];
	for (int i = 0; i < nThreads; i++)
	{
		dataI[i] = new uint8_t[(nTilesPerRow * 64 + 2 * 64) * (tilesRowPerThread * 64 + 64 * 2)];
	}
	lDebug(1, "Allocating %llu bytes for %i threads", (nTilesPerRow * 64 + 2 * 64) * (tilesRowPerThread * 64 + 64 * 2), nThreads);
	lDebug(1, "%llu, Allocating in a matrix of %i x %i tiles", n, (tilesRowPerThread + 2), (nTilesPerRow + 2));
...
void AMXSolver::CAStepAlgorithm()
{
	...
	size_t n = dataDomain->getInnerHorizontalSize();
	int nTilesPerRow = n / 64;
	int tilesRowPerThread = (nTilesPerRow + num_threads - 1) / num_threads;

	int start = 0;
	int end = tilesRowPerThread + 2;

	// int end = chunk_size;
	// lDebug(1, "First: Thread %i, start %i, end %i, nTilesPerRow %i, tilesRowPerThread %i\n", thread_id, start, end, nTilesPerRow, tilesRowPerThread);
	if ((thread_id + 1) * 64 > n)
	{
	...
	{
		size_t i = iter * 64;
		if ((thread_id * tilesRowPerThread * 64 + i) >= nWithHalo)
			continue;

		// check if the thread data is out of bounds
		...
		for (size_t j = 0; j < nWithHalo - 64 * 2; j += 64)
		{
			...
			{
				_tile_zero(0);
				_tile_loadd(7, &data[(thread_id * tilesRowPerThread * 64 + i + 16 * k) * nWithHalo + j], nWithHalo);
				_tile_dpbssd(0, 7, 6);
				_tile_loadd(7, &data[(thread_id * tilesRowPerThread * 64 + i + 16 * k) * nWithHalo + j + 64], nWithHalo);
				_tile_dpbuud(0, 7, 2);
			...
			{
				_tile_zero(0);
				_tile_loadd(7, &data[(thread_id * tilesRowPerThread * 64 + i + 16 * k) * nWithHalo + j + 128], nWithHalo);
				_tile_dpbssd(0, 7, 1);
				_tile_loadd(7, &data[(thread_id * tilesRowPerThread * 64 + i + 16 * k) * nWithHalo + j + 64], nWithHalo);
				_tile_dpbssd(0, 7, 5);
			...
			for (int ii = 0; ii < 64; ii++)
			{
				...
				{
					dataI[thread_id][(i + jj) * nWithHalo + j + ii + 64] = buffer[thread_id * 64 * 64 + ii * 64 + jj];
				}
			}
	...
	// if ((thread_id + 1) * 64 > n)
	// {
	// 	printf("SECOND Thread %i is out of bounds\n", thread_id);
	// 	return;
	// }
	...
	start = 0;
	end = tilesRowPerThread;
	// lDebug(1, "SECOND: Thread %i, start %i, end %i, nTilesPerRow %i, tilesRowPerThread %i\n", thread_id, start, end, nTilesPerRow, tilesRowPerThread);
	// end = (nTilesPerRow % num_threads != 0) ? (tilesRowPerThread) - (nTilesPerRow % num_threads) : tilesRowPerThread;

	// if (nTilesPerRow % num_threads != 0)
	// {
	// 	printf("Number of threads must be a multiple of the number of tiles per row\n");
	// 	// return;
	// }
	...
	{
		size_t i = iter * 64;
		if ((thread_id * tilesRowPerThread * 64 + i + 128) >= nWithHalo)
			continue;
		...
		for (int j = 0; j < nWithHalo - 64 * 2; j += 64)
		{
			...
			{
				_tile_zero(0);
				_tile_loadd(7, &dataI[thread_id][(i + 16 * k) * nWithHalo + j + 64], nWithHalo);
				_tile_dpbuud(0, 7, 6);
				_tile_loadd(7, &dataI[thread_id][(i + 64 + 16 * k) * nWithHalo + j + 64], nWithHalo);
				_tile_dpbuud(0, 7, 2);
				_tile_stored(0, &buffer[thread_id * 64 * 64 + (k * 16) * 64 + 16 * 0], 64 * 4);
			...
			{
				_tile_zero(0);
				_tile_loadd(7, &dataI[thread_id][(i + 128 + 16 * k) * nWithHalo + j + 64], nWithHalo);
				_tile_dpbssd(0, 7, 1);
				_tile_loadd(7, &dataI[thread_id][(i + 64 + 16 * k) * nWithHalo + j + 64], nWithHalo);
				_tile_dpbssd(0, 7, 5);
				_tile_stored(0, &buffer[thread_id * 64 * 64 + k * 16 * 64 + 16 * 3], 16 * 4 * 4);
			...
			for (int ii = 0; ii < 64; ii++)
			{
				...
				{
					uint8_t cellValue = data[(thread_id * tilesRowPerThread * 64 + i + ii + 64) * nWithHalo + j + 64 + jj];
					...
					dataBuffer[(thread_id * tilesRowPerThread * 64 + i + ii + 64) * nWithHalo + j + 64 + jj] = result;
					// dataBuffer[(i+ii + 64)*nWithHalo + j + 64 + jj] = liveNeighbors;
